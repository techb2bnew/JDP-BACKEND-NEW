...
CREATE TABLE IF NOT EXISTS jobs (
  ...
  work_activity          INTEGER DEFAULT 0,
...
COMMENT ON TABLE jobs IS 'Main jobs table with work activity tracking and timer fields';
COMMENT ON COLUMN jobs.work_activity IS 'Number of work activities (simple count)';
COMMENT ON COLUMN jobs.total_work_time IS 'Total work time as INTERVAL';
COMMENT ON COLUMN jobs.start_timer IS 'Job start timestamp';
COMMENT ON COLUMN jobs.end_timer IS 'Job end timestamp';
COMMENT ON COLUMN jobs.pause_timer IS 'Array of pause objects with title and duration';
