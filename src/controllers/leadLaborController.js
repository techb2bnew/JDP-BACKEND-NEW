...
export class LeadLaborController {
  ...
  static async updateLeadLabor(req, reply) {
    try {
      ...
      return reply.status(500).send(errorResponse(`Failed to update lead labor: ${error.message}`, 500));
    }
  }
  ...
  static async importLeadLabor(request, reply) {
    try {
      ...
      const result = await LeadLaborService.importLeadLabor(csvContent, userId);
      ...
      return reply.status(500).send(errorResponse(
        `Failed to import lead labor: ${error.message}`,
        500
      ));
    }
  }
