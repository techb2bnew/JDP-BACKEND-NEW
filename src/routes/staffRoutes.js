...
export default async function staffRoutes(fastify, options) {
  ...
  fastify.post('/updateProfileImage/:staffId', async (request, reply) => {
    if (request.isMultipart()) {
      const body = {};
      const files = {};
      ...
      for await (const part of parts) {
        if (part.type === 'file') {
          ...
          try {
            const buffer = await part.toBuffer();
            const fileName = `staff/profile_image/${Date.now()}-${part.filename}`;
    ...
    return StaffController.updateProfileImage(request, reply);
  });
